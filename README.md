...
Last updated for v0.5.8 (Apr 2026)
...
3. Delete the following files from the `mods` folder on the server (the new one in this folder). These are client-side mods that do not disable themselves on the server and therefore crash the server if left untouched:
   Alternatively, run `delete_client_mods.bat` on Windows to do this step for you, or `delete_client_mods.sh` if you're on Mac or Linux.
...
4. Optional: set the JVM arguments as desired, you might need more RAM than the amount I set. `-Xmx`_(NUMBER GOES HERE)_`G` to set maximum allocated memory.
